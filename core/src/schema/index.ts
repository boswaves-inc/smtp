...
export const EmailLog = pgTable('email_logs', t => ({
    ...
    status: t.varchar({ length: 50 }).default("pending"), // pending, sent, failed, delivered, bounced, complained
    ses_message_id: t.text(),
    error_message: t.text(),
    webhook_data: t.jsonb(),
    created_at: t.timestamp({ withTimezone: true }).defaultNow(),
    updated_at: t.timestamp({ withTimezone: true }).defaultNow().$onUpdate(() => new Date()),
}), table => [
