...
export default ({ logger, postgres, smtp }: Context) => {
    ...
    return async ({ message }: KafkaMessage) => {
        ...
        try {
            ...
            const idempotency_key = req_idempotency_key(body)

            logger.info({ idempotency_key, body }, 'email-queued')

            const existing = await postgres.query.Email.findFirst({
                where: eq(Email.idempotency_key, idempotency_key)
            })

            if (existing) {
                return logger.info({ idempotency_key }, 'duplicate, skipping')
            }
